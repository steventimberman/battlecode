...
public class WalkieTalkie {
  ...
  static final String[] messasgeType = {"Make Vaporator", "VaporatorMade"};
  ...
  public static void sendVaporatorMade() throws GameActionException{
    int[] message = genericMessage(1);
    sendMessage(message, 3);
  }

  public static boolean receiveVaporatorMade() throws GameActionException{
    int currentRound = rc.getRoundNum();
    for (Transaction trans : rc.getBlock(currentRound - 1)){
      int[] message = trans.getMessage();
      if (message[0] == teamSecretKey && message[1] == 0){
          if (message[2] != rc.getID()){
            return true;
          }
      }
    }
